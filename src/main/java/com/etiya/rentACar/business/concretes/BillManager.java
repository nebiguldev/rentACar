...
public class BillManager implements BillService {
    ...
    //aracı aldığımız ve bıraktığımız gün arasındaki
    @Override
    ...
    public DataResult<List<ListBillDto>> getAll() {
        ...
        List<ListBillDto> responce = bills.stream().map(bill -> this.modelMapperService.forDto().map(bill, ListBillDto.class)).collect(Collectors.toList());


        return new SuccessDataResult<List<ListBillDto>>(responce);


    }
    private double calculateTotalPrice(RentalDto rentalDto) {
        double totalPrice = 0;
        //iki tarih arasında ve farklı şehirde bırakılacaksa(different location) ek ücret 750 TL
        Period day = Period.between(rentalDto.getRentDate(), rentalDto.getReturnDate());
        int daysCount = day.getDays();
